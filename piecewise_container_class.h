...
public:
    ...
    piecewise_container_class(short k, int count_of_collocations, bool random_number) : container_class_interface(k, count_of_collocations)
    {
        ...
        auto zapisey_in_one_db = 4000000;
        int _commiter = 10000;
        unsigned long long vsego = pow(count_of_collocations, 2) * k;
        int number_of_full_db = vsego / zapisey_in_one_db; //4000000 now fastest
        int terms_in_one_db = zapisey_in_one_db / count_of_collocations / k;
        int tails = number_of_full_db * terms_in_one_db;
        int rc;
        string textname;

        //cout << vsego << " " << number_of_full_db << " " << terms_in_one_db << " " << tails << endl;
        //cout << coc * coc * k;
        ...
        for (int t = 0; t < number_of_full_db; t++)
        {
            ...
            textname = "./text" + to_string(text_counter) + " terms " + to_string(left_term) + "-" + to_string(right_term);
        ...
        if (tails != count_of_collocations) {
            textname = "./text" + to_string(text_counter) + " terms " + to_string(tails) + "-" + to_string(count_of_collocations);

            if (fs::exists(textname))
                fs::remove(textname);//debug

            this->_filenames.insert(make_pair(textname, make_pair(tails, count_of_collocations)));
            ////////// СОЗДАНИЕ ФАЙЛА БД
            rc = mdbx_env_create(&env);
            if (rc != MDBX_SUCCESS) {
                fprintf(stderr, "mdbx_env_create: (%d) %s\n", rc, mdbx_strerror(rc));
