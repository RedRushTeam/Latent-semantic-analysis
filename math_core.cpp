...
void math_core::find_SVD_terms()
{
	...
	this->cosinuses.clear();
	std::cout.flush();
	cout << endl << "Количество значимых термов после разложения и свертки tf/idf, с множителем " << global_var::KOEF_FOR_COLLOC_COS_DELETE << " :" << this->set_for_unique_terms->size();
